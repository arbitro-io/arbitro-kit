...
//!
//! The kit-native analog of `tokio::sync::oneshot`. Built directly on
//! [`Signal`](crate::gate::Signal) + a `MaybeUninit<T>` slot, with an
//! atomic state machine (`empty` → `full` | `closed`) so the receiver
//! can distinguish "value sent" from "sender dropped".
//!
...
//! ## Cost model
//!
//! Identical to a `Pipe<T>` cycle plus one `AtomicU8` transition:
//! - `send` (consumer not parked): one Release store + one Signal release.
//! - `send` (consumer parked):     same + one `Thread::unpark` syscall.
//! - `recv` (value already there): one Acquire load on state.
//! - `recv` (value not there):     one `std::thread::park` syscall round-trip.
//!
//! ## Drop safety
//!
...
// by receiver). The Signal handles the cross-thread visibility.
...
impl<T: Send> Sender<T> {
    ...
    /// `Signal::release` → `Thread::unpark` if parked.
    #[inline]
    pub fn send(mut self, value: T) {
        // Safety: state is EMPTY (we are the only sender, no concurrent
        // writer to the slot exists yet); releasing state to FULL after
        // the write makes the slot's contents visible to the receiver.
        unsafe { (*self.inner.slot.get()).write(value); }
        self.inner.state.store(STATE_FULL, Ordering::Release);
        self.inner.signal.release();
        self.sent = true;
    }
}
...
impl<T: Send> Drop for Sender<T> {
    fn drop(&mut self) {
        if !self.sent {
            // Mark closed BEFORE waking, so the receiver's recheck
...
impl<T: Send> Receiver<T> {
    ...
    #[inline]
    pub fn bind(&self) {
        self.inner.signal.set_worker(std::thread::current());
    }
    ...
    pub fn recv(self) -> Result<T, Closed> {
        loop {
            match self.try_take() {
                Some(r) => return r,
                None => self.inner.signal.acquire(),
            }
        }
    }
