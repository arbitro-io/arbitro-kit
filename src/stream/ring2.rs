//! SPSC bounded ring buffer — split-handle variant (Ring2 v2).
//!
//! [`Ring2::new`] returns a `(Producer, Consumer)` pair (rtrb /
...
//! reference drops, [`Ring2`]'s own `Drop` drains any in-flight `T`
...
/// by users — [`Ring2::new`] returns the [`Producer`] / [`Consumer`]
...
/// Sending half of a [`Ring2`]. Uniquely owned: `Send` but neither
...
impl<T, const CAP: usize, W: Waiter> Producer<T, CAP, W> {
    ...
    pub fn try_send(&mut self, value: T) -> Result<(), TrySendError<T>> {
        ...
        unsafe {
            (*shared.slots[head & Ring2::<T, CAP, W>::MASK].get()).write(value);
        }
...
/// Receiving half of a [`Ring2`]. Uniquely owned: `Send` but neither
...
impl<T, const CAP: usize, W: Waiter> Consumer<T, CAP, W> {
    ...
    pub fn try_recv(&mut self) -> Result<T, TryRecvError> {
        ...
        let v = unsafe {
            (*shared.slots[tail & Ring2::<T, CAP, W>::MASK].get()).assume_init_read()
        };
...
mod tests {
    ...
    fn single_thread_basic() {
        let (mut tx, mut rx) = Ring2::<u32, 8>::new();
        assert!(rx.is_empty());
    ...
    #[test]
    fn wraparound() {
        let (mut tx, mut rx) = Ring2::<u32, 4>::new();
        for i in 0..100 {
            assert!(tx.try_send(i).is_ok());
            assert_eq!(rx.try_recv(), Ok(i));
        }
    ...
    fn cross_thread_blocking() {
        let (mut tx, mut rx) = Ring2::<u64, 16>::new();
    ...
    fn cross_thread_backpressure() {
        ...
        const CAP: usize = 4;
        const N: u64 = 500;
        let (mut tx, mut rx) = Ring2::<u64, CAP>::new();
    ...
    fn drop_drains_inflight() {
        ...
        {
            let (mut tx, rx) = Ring2::<Tracked, 8>::new();
            for _ in 0..5 {
    ...
    fn handle_moves_between_threads() {
        ...
        let (mut tx, mut rx) = Ring2::<u64, 4>::new();
        tx.send(1).unwrap();
        assert_eq!(rx.recv(), Some(1));
        let h = std::thread::spawn(move || {
            let v = rx.recv();
    ...
    async fn cross_task_basic_notify() {
        ...
        let (mut tx, mut rx) = Ring2::<u64, 16, NotifyWaiter>::new();
        let producer = async {
            for i in 0..1000u64 {
    ...
    async fn wraparound_notify() {
        use crate::waiter::NotifyWaiter;
        let (mut tx, mut rx) = Ring2::<u32, 4, NotifyWaiter>::new();
        for i in 0..100 {
            tx.send_async(i).await.unwrap();
    ...
    async fn producer_drop_disconnects_async_consumer() {
        use crate::waiter::NotifyWaiter;
        let (mut tx, mut rx) = Ring2::<u32, 8, NotifyWaiter>::new();
