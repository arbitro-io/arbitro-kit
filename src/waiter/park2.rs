//! `ParkWaiter2` — hardened OS-thread `Waiter` (lost-wake-proof T3a/T3b probe + atomic worker registration). A/B sibling of `ParkWaiter` (park.rs), which stays untouched as baseline.
//!
...
impl ParkWaiter2 {
    /// Construct a `ParkWaiter2` with a custom spin budget. Higher = lower
    ...
    pub fn wait_until_deadline<F: FnMut() -> bool>(&self, deadline: Instant, mut ready: F) -> bool {
        ...
        assert!(
            self.has_worker(),
            "ParkWaiter2::wait_until_deadline reached park path without set_worker — register the consumer thread first",
        );
        // Tight spin.
        for _ in 0..TIGHT_SPIN {
            if ready() {
...
impl BlockingWaiter for ParkWaiter2 {
...
mod tests {
    ...
    fn fast_path_already_ready() {
        let w = ParkWaiter2::default();
        w.wait_until(|| true);
    }

    #[test]
    fn wake_after_state_change_releases_parked_thread() {
        let w = Arc::new(ParkWaiter2::default());
    ...
    /// through a real consumer — `Ring2<u32, 1>` (default `ParkWaiter2`)
